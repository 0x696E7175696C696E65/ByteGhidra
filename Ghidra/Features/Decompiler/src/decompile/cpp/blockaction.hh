...
class ActionStructureTransform : public Action {
  ...
  bool qualityGated;		///< If true, allowOpMoves is controlled by Architecture::pseudocode_quality
public:
  ActionStructureTransform(const string &g,bool allowMoves,bool quality=false) : Action(0,"structuretransform",g) { allowOpMoves = allowMoves; qualityGated = quality; }	///< Constructor
  virtual Action *clone(const ActionGroupList &grouplist) const {
    if (!grouplist.contains(getGroup())) return (Action *)0;
    return new ActionStructureTransform(getGroup(),allowOpMoves,qualityGated);
  }
...
class ActionPreferComplement : public Action {
  ...
  bool qualityGated;		///< If true, allowOpMods is controlled by Architecture::pseudocode_quality
public:
  ActionPreferComplement(const string &g,bool allowMods,bool quality=false) : Action(0,"prefercomplement",g) { allowOpMods = allowMods; qualityGated = quality; }	///< Constructor
  virtual Action *clone(const ActionGroupList &grouplist) const {
    if (!grouplist.contains(getGroup())) return (Action *)0;
    return new ActionPreferComplement(getGroup(),allowOpMods,qualityGated);
  }
  virtual int4 apply(Funcdata &data);
};
