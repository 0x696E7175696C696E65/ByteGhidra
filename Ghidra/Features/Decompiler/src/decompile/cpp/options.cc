...
ElementId ELEM_PSEUDOCODEQUALITY = ElementId("pseudocodequality",291);
...

{
  ...
  registerOption(new OptionPseudocodeQuality());
}
...
/// \class OptionPseudocodeQuality
/// \brief Toggle enriched pseudocode cleanup and recovery behavior
///
/// The first parameter is "on" or "off".  The flag is enabled by default and gates
/// extra native structuring passes that can move or rewrite p-code late in recovery.
string OptionPseudocodeQuality::apply(Architecture *glb,const string &p1,const string &p2,const string &p3) const

{
  if (p1.size()==0)
    throw ParseError("Pseudocode quality option must be set \"on\" or \"off\"");
  bool oldMode = glb->pseudocode_quality;
  glb->pseudocode_quality = onOrOff(p1);
  if (oldMode == glb->pseudocode_quality)
    return "Pseudocode quality mode unchanged";
  if (glb->pseudocode_quality)
    return "Pseudocode quality mode enabled";
  return "Pseudocode quality mode disabled";
}
