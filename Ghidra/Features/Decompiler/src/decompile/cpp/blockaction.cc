...
{
  bool allowMoves = allowOpMoves && (!qualityGated || data.getArch()->pseudocode_quality);
  data.getStructure().finalTransform(data,allowMoves);
  return 0;
}
...

{
  ...
  bool allowMods = allowOpMods && (!qualityGated || data.getArch()->pseudocode_quality);
  ...
  while(pos < vec.size()) {
    ...
    if (curbl->preferComplement(data,allowMods))
      count += 1;
  }
