...
public class CodeBrowserPlugin extends AbstractCodeBrowserPlugin<CodeViewerProvider>
	...
	public CodeBrowserPlugin(PluginTool tool) {
		...
		GameAnalysisOptions.register(tool);
		createCrackmeAuthCheckAction();
		createGameAnalysisAction();
	}

	private void createCrackmeAuthCheckAction() {
		new ActionBuilder("Search For Crackme Auth Checks", getName())
				.onAction(c -> showCrackmeAuthChecks())
				.menuPath(ToolConstants.MENU_SEARCH, "For Crackme Auth Checks")
				.menuGroup("search for", "zz")
				.helpLocation(new HelpLocation("CodeBrowserPlugin", "Crackme_Auth_Checks"))
				.buildAndInstall(tool);
	}

	private void showCrackmeAuthChecks() {
		...
		if (program == null) {
			tool.setStatusInfo("Unable to find crackme auth checks: no active program");
			return;
		}

		CrackmeAuthCheckTableModel model = new CrackmeAuthCheckTableModel(tool, program);
		Icon markerIcon = new GIcon("icon.plugin.codebrowser.cursor.marker");
		String title = "Crackme Auth Checks in " + program.getName();
		TableComponentProvider<CrackmeAuthCheckCandidate> tableProvider =
			tableService.showTableWithMarkers(title, "Crackme Auth Checks", model,
				SearchConstants.SEARCH_HIGHLIGHT_COLOR, markerIcon, title, connectedProvider);
		tableProvider.installRemoveItemsAction();
	}

	private void createGameAnalysisAction() {
		new ActionBuilder("Search For Game Logic/Data Structures", getName())
				.onAction(c -> showGameAnalysisResults())
				.menuPath(ToolConstants.MENU_SEARCH, "For Game Logic/Data Structures...")
				.menuGroup("search for", "zz")
				.helpLocation(new HelpLocation("CodeBrowserPlugin", "Game_Analysis"))
				.buildAndInstall(tool);
	}

	private void showGameAnalysisResults() {
		TableService tableService = tool.getService(TableService.class);
		if (tableService == null) {
			Msg.showWarn(this, null, "No Table Service", "Please add the TableServicePlugin.");
			return;
		}
		if (!GameAnalysisOptions.confirmOfflineUse(tool, tool.getToolFrame())) {
			tool.setStatusInfo("Game analysis search canceled");
			return;
		}

		Program program = currentProgram;
		if (program == null) {
			tool.setStatusInfo("Unable to find game logic/data structures: no active program");
			return;
		}

		GameAnalysisTableModel model = new GameAnalysisTableModel(tool, program);
		Icon markerIcon = new GIcon("icon.plugin.codebrowser.cursor.marker");
		String title = "Game Logic/Data Structures in " + program.getName() +
			" (offline single-player analysis only)";
		TableComponentProvider<GameAnalysisCandidate> tableProvider =
			tableService.showTableWithMarkers(title, "Game Analysis", model,
				SearchConstants.SEARCH_HIGHLIGHT_COLOR, markerIcon, "Game Analysis",
				connectedProvider);
		tableProvider.installRemoveItemsAction();
	}
